...
while answer == 'yes':
    ...
    h_num_dict_third_digit = {1: 'one-hundred', 2: 'two-hundred', 3: 'three-hundred', 4: 'four-hundred', 5: 'five-hundred', 6: 'six-hundred', 7: 'seven-hundred', 8: 'eight-hundred', 9: 'nine-hundred'}
    ...
    if low_user_input > 99:
        ...
        # hundreds place
        hundreds_place = h_num_dict_third_digit[hundreds_digit]

        # tens place
        if h_num_tens == 0:
            tens_place = ''
        elif h_num_tens > 1:
            tens_place = h_num_dict_second_digit[tens_digit]

        #ones place and teens
        if h_num_tens == 1:
            ones_place = h_num_dict_teens[ones_digit]
        else:
            if h_num_ones > 0:
                ones_place = h_num_dict_ones[ones_digit]
            else:
                ones_place = ' '

        # printing
        if h_num_tens > 1:
            print(hundreds_place + '-' + tens_place + '-' + ones_place)
